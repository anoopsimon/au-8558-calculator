document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('addStayPeriod').addEventListener('click', function() {
        ...
        newPeriod.querySelectorAll('input').forEach(input => {
            input.value = '';document.addEventListener('DOMContentLoaded', function() {
                const form = document.getElementById('stayPeriods');
                document.getElementById('addStayPeriod').addEventListener('click', function() {
                    const newPeriod = form.firstElementChild.cloneNode(true);
                    newPeriod.querySelectorAll('input').forEach(input => {
                        input.value = '';
                        addDateInputMask(input); // Ensure masks are applied to new inputs
                    });
                ...
                document.getElementById('calculate').addEventListener('click', function() {
                    ...
                    const className = totalDaysInside > 365 ? 'result overstay' : 'result legal-stay';
                 document.getElementById('result').className = className;
                    //document.getElementById('result').className = totalDaysInside > 365 ? 'result overstay' : 'result legal-stay';
                    document.getElementById('result').textContent = `${resultText}. Total days stayed in Australia: ${Math.round(totalDaysInside)}. Calculation starts from: ${formatDate(eighteenMonthsAgo)}`;
                });
            });
